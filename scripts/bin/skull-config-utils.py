...
PB_HEADER = """// This is generated by framework
// Notes:
//   - The top message is the main message structure we use,
//      if we want to define some sub-messages, write then
//      after top message
//   - For pb2, Do not recommend to use 'required' field,
//      if we really want to use it, make sure to fill it
//      before using it

syntax = "proto3";
package skull.workflow;
...
def _create_workflow():
    return {'concurrency': 1, 'idl': "", 'stdin': 0, 'port': -1, 'modules': []}
...
def _process_gen_workflow_idl():
    ...
    for workflow in workflows:
        ...
        content = PB_HEADER.format(idl_name)
...
def usage():
    ...
    print(
        "  skull-config-utils.py -m workflow -c $yaml_file -a add -C"
        " $concurrency -i $IDL_name -p $port"
    )
    print(
        "  skull-config-utils.py -m workflow -c $yaml_file -a gen_idl -n"
        " $IDL_name -p $IDL_path"
    )

    print(
        "  skull-config-utils.py -m module   -c $yaml_file -a add -M"
        " $module_name -i $workflow_index"
    )

    print("  skull-config-utils.py -m service  -c $yaml_file -a show")
    print(
        "  skull-config-utils.py -m service  -c $yaml_file -a exist -s"
        " $service_name"
    )
    print(
        "  skull-config-utils.py -m service  -c $yaml_file -a add -s"
        " $service_name -b $enabled -d $data_mode -l language"
    )
