...
static
int svc_api_callback1(skullcpp::Txn& txn, skullcpp::Txn::IOStatus status,
    ...
    std::cout << "svc_api_callback1.... apiName: " << apiName << std::endl;
    std::cout << "svc_api_callback1.... request: " << apiReq.name() << std::endl;
    std::cout << "svc_api_callback1.... response: " << apiResp.response() << std::endl;
                     ...
                     const google::protobuf::Message& response)
{
    ...
    // Call service in the callback function
...
void module_pack(skullcpp::Txn& txn, skullcpp::TxnData& txndata)
{
    ...
    if (txn.status() != skullcpp::Txn::TXN_OK) {
        SKULL_LOG_ERROR("5", "module_pack(test): error status occurred. txn data: %s", example.data().c_str(), "This error is expected");
