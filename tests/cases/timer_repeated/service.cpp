...
static
void _timerjob(skullcpp::Service& service, int times) {
    std::cout << "timer job triggered" << std::endl;
    SKULL_LOG_INFO("svc.test.timerjob", "timer job triggered %d", times);

    if (times < 5) {
        // Create a timer job again
        service.createJob(100, 1, skull_BindSvc(_timerjob, times + 1));
    }
}
