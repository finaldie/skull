...
# Global message factory (Notes: Should not create it dynamically, or it will
# lead memleak)
...
class Txn():
    ...
    def iocall(self,
        ...
        @param api_cb        api callback function.
                             prototype: func(txn, iostatus, apiName,
        ...
        if request_msg is None or isinstance(request_msg,
                                             message.Message) is False:
            return Txn.IO_ERROR_REQUEST

        if api_cb is not None and isinstance(api_cb,
        ...
        return capi.txn_iocall(self._skull_txn, service_name, api_name,
                               msg_bin_data, bio_idx, _service_api_callback,
                               api_cb)
    ...
    def __get_or_create_message(self, proto_full_name):
        ...
        proto_descriptor = _MESSAGE_FACTORY.pool.FindMessageTypeByName(
            proto_full_name)
