...
int _run(sk_sched_t* sched, sk_entity_t* entity, sk_txn_t* txn, void* proto_msg)
{
    ...
    if (!service) {
        ...
        srv_status = SK_SRV_STATUS_INVALID_SRV_NAME;
    }
    ...
    task.req_type = (sk_srv_req_type_t) data_access_mode;
    task.srv_status = srv_status;
    task.request = req_data;
    task.request_sz = req_data_sz;

    // 3.2 push to service
    int ret = 1;
    task.srv_status = sk_service_push_task(service, &task);

    if (task.srv_status == SK_SRV_STATUS_BUSY ||
        task.srv_status == SK_SRV_STATUS_INVALID_SRV_NAME) {
    ...
    ret = sk_service_schedule_tasks(service);
    SK_LOG_ERROR(SK_ENV_LOGGER, "Service Iocall failed, service name %s",
                 service_name);
